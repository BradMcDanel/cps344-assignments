class Router:
    ...
    def forward(self, ip_addr):
        # TODO: send the ip_addr (packet) to the correct neighbor based on the forwarding table
        #       A forwarding table can have a single '*' key, which is used if no other key matches
        # temporary code (replace)
        for key in self.forward_table.keys():
